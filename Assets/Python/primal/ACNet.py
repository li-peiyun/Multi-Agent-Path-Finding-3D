...
class ACNet:
    def __init__(self, scope, a_size, trainer,TRAINING,GRID_SIZE,GLOBAL_NET_SCOPE):
        ...
        if TRAINING:
            ...
            self.entropy       = - tf.reduce_sum(self.policy * tf.compat.v1.log(tf.clip_by_value(self.policy,1e-10,1.0)))
            self.policy_loss   = - tf.reduce_sum(tf.compat.v1.log(tf.clip_by_value(self.responsible_outputs,1e-15,1.0)) * self.advantages)
            self.valid_loss    = - tf.reduce_sum(tf.compat.v1.log(tf.clip_by_value(self.valids,1e-10,1.0)) *\
                                self.train_valid+tf.compat.v1.log(tf.clip_by_value(1-self.valids,1e-10,1.0)) * (1-self.train_valid))
            self.blocking_loss = - tf.reduce_sum(self.target_blockings*tf.compat.v1.log(tf.clip_by_value(self.blocking,1e-10,1.0))\
                                      +(1-self.target_blockings)*tf.compat.v1.log(tf.clip_by_value(1-self.blocking,1e-10,1.0)))
            self.on_goal_loss = - tf.reduce_sum(self.target_on_goals*tf.compat.v1.log(tf.clip_by_value(self.on_goal,1e-10,1.0))\
                                      +(1-self.target_on_goals)*tf.compat.v1.log(tf.clip_by_value(1-self.on_goal,1e-10,1.0)))
